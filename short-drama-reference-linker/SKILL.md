...
多人 shot 要先通過「人物數量門檻」。如果 `reference_assignment` 使用兩個 character anchors，且本格核心是求助、對峙、追問、交接、一起躲避或關係壓力，`start_image_prompt` 必須在 `改成...` 後明確寫出兩個人物都在畫面內，並交代左右、前後或近遠位置。只出現一個人物、另一人只剩一隻手或半個身體邊緣，除非本格明確是主觀視角或偷看視角，否則視為失敗構圖。
...
當本格使用 `1 scene + 2 characters` 時，`改成...` 後的第一句應先鎖定雙人站位，再寫道具或情緒。例如 `改成臥室中近景，女孩在畫面左側，中年女性在畫面右側，兩人都在畫面內...`。不要只在前面的 reference 保留句提到第二個角色，卻在真正的首幀描述裡只安排一個角色。
